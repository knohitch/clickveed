...
export default function SupportPage() {
    ...
    useEffect(() => {
        const fetchTickets = async () => {
            ...
            if (ticketIdFromUrl && fetchedTickets.some(t => t.id === ticketIdFromUrl)) {
    ...
    useEffect(() => {
        if (!loading && selectedTicketId && !filteredTickets.some(t => t.id === selectedTicketId)) {
            setSelectedTicketId(filteredTickets[0]?.id || null);
        }
    }, [searchQuery, selectedTicketId, filteredTickets, loading]);
                                    ...
                                    {selectedTicket.conversation.map((msg, index) => (
                                        <div key={index} className={cn("flex items-start gap-4", msg.sender === 'agent' && "justify-end")}>
                                            ...
                                            <div className={cn("max-w-lg rounded-lg px-4 py-3", msg.sender === 'user' ? "bg-muted" : "bg-primary text-primary-foreground")}>
                                                ...
                                                <p className="text-xs text-right mt-2 opacity-70">{new Date(msg.timestamp).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</p>
                                            </div>
                            ...
                            <div className="p-4 border-t bg-background">
                                <div className="relative">
                                     <Textarea 
                                        ...
                                        disabled={isReplying}
                                    />
                                     <Button size="icon" className="absolute right-3 bottom-2 h-8 w-8" onClick={handleReply} disabled={!replyText.trim() || isReplying}>
