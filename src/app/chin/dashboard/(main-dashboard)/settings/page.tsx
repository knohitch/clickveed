...
export default function AdminSettingsPage() {
    ...
    const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setIsSaving(true);
        try {
            // These functions update both state AND save to database
            await setAppName(localSettings.appName);
            await setEmailSettings(localSettings.emailSettings);
            await setApiKeys(localSettings.apiKeys);

            toast({ title: "Settings Saved", description: "All settings have been updated successfully." });
        } catch (error: any) {
            console.error('[Settings] Error saving:', error);
            toast({ variant: 'destructive', title: "Error Saving Settings", description: error.message || "Failed to save settings. Please try again." });
        } finally {
            setIsSaving(false);
        }
    };
