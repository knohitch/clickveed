...
const defaultEmailTemplates: EmailTemplates = {
    userSignup: { subject: 'Welcome to {{appName}}!', body: '<h1>Welcome {{name}}!</h1><p>Thank you for signing up with {{appName}}. We\'re excited to have you on board.</p>' },
    passwordReset: { subject: 'Password Reset Request', body: '<h1>Password Reset</h1><p>Click <a href="{{resetLink}}">here</a> to reset your password.</p>' },
    subscriptionActivated: { subject: 'Subscription Activated', body: '<h1>Welcome to {{planName}}!</h1><p>Your subscription to {{planName}} has been activated. Thank you for choosing {{appName}}!</p>' },
    subscriptionRenewal: { subject: 'Subscription Renewal Reminder', body: '<h1>Subscription Renewal</h1><p>Your {{planName}} subscription will renew on {{renewalDate}}. Make sure your payment method is up to date.</p>' },
    subscriptionCanceled: { subject: 'Subscription Canceled', body: '<h1>Subscription Canceled</h1><p>Your {{planName}} subscription has been canceled and will end on {{endDate}}.</p>' },
    adminNewUser: { subject: 'New User Signup', body: '<h1>New User</h1><p>A new user has signed up: {{userEmail}}</p>' },
    adminSubscriptionCanceled: { subject: 'Subscription Canceled', body: '<h1>Subscription Canceled</h1><p>{{userName}} has canceled their {{planName}} subscription.</p>' },
    adminSubscriptionRenewed: { subject: 'Subscription Renewed', body: '<h1>Subscription Renewed</h1><p>{{userName}} has renewed their {{planName}} subscription.</p>' },
    userNewTicket: { subject: 'Support Ticket Received', body: '<h1>Support Ticket</h1><p>We\'ve received your ticket #{{ticketId}}: {{ticketSubject}}. Our team will respond shortly.</p>' },
    userTicketReply: { subject: 'Reply to your Support Ticket #{{ticketId}}', body: '<h1>Ticket Update</h1><p>Your support ticket has a new reply: {{replyMessage}}</p>' },
    userTicketStatusChange: { subject: 'Ticket Status Updated', body: '<h1>Ticket Update</h1><p>Your ticket #{{ticketId}} status has been updated to {{newStatus}}.</p>' },
    adminNewTicket: { subject: 'New Support Ticket', body: '<h1>New Ticket</h1><p>{{userName}} ({{userEmail}}) has submitted a new ticket #{{ticketId}}: {{ticketSubject}}</p>' },
};
...
export default function ChinEmailTemplatesPage() {
    const { toast } = useToast();
    const [localTemplates, setLocalTemplates] = useState<EmailTemplates>(defaultEmailTemplates);
    ...
    const handleSaveChanges = () => {
        ...
        console.log('Saving templates:', localTemplates);
