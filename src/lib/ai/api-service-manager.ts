...
export async function generateWithProvider(req: Omit<GenerateRequest, 'model'>) {
    // Priority order: try OpenAI first, then Claude, then others
    const providerPriority = ['openai', 'azureOpenai', 'claude', 'deepseek', 'grok', 'qwen', 'perplexity', 'openrouter', 'huggingface'];
    ...
    for (const providerName of providerPriority) {
        ...
        try {
            ...
            return {
                model: providerName === 'claude' ? 'anthropic/claude-3-5-sonnet' : 'openai/gpt-4o',
                result: {
                    role: 'model' as const,
    ...
    if (allProviders.length === 0) {
        throw new Error('No AI provider API keys configured. Please add an API key in admin settings.');
    }
...
export async function generateStreamWithProvider(req: Omit<GenerateStreamRequest, 'model'>) {
    // Priority order for streaming: OpenAI first, then Claude
    const providerPriority = ['openai', 'azureOpenai', 'claude'];
    ...
    throw new Error('No streaming provider available. Please configure an API key.');
}
