...
import { type MediaAsset as MediaAssetType, deleteMediaAsset } from '@/lib/media-actions';
...
export function MediaLibraryClient({ initialAssets }: { initialAssets: MediaAssetType[] }) {
    const { toast } = useToast();
    const [assets, setAssets] = useState<MediaAssetType[]>(initialAssets);
    ...
    const handleDeleteAsset = async (assetId: number) => {
        ...
        try {
            ...
            toast({ title: 'Media Deleted', description: 'The media item was removed from your library.' });
        } catch (error) {
            toast({
                variant: 'destructive',
                title: 'Delete Failed',
